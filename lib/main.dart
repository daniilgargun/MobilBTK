...
void main() {
  ...
  runZonedGuarded(
    () async {
      ...
      if (firebaseReady) {
        unawaited(
          remoteConfig.refresh().then((_) async {
            LessonTime.applyRemoteOverride(
              remoteConfig.config.bellScheduleJson,
            );

            // Подписку сверяем ещё раз, уже по свежему конфигу.
            //
            // Выше `initialize` отработал на значениях с прошлого запуска:
            // конфиг тянется из сети и к тому моменту ещё не пришёл. Без
            // повторной сверки `push_enabled` вступал бы в силу только со
            // второго открытия приложения — а это аварийный выключатель,
            // от него ждут немедленного действия. Проверено на устройстве:
            // после включения в консоли подписка появлялась лишь со
            // второго запуска.
