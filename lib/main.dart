...
import 'dart:async';
import 'dart:io';
import 'minigame/screens/minigame_screen.dart';
...
class MyHomePageState extends State<MyHomePage> {
  ...
  bool _hideMinigame = false; // Добавляем переменную для отслеживания скрытия мини-игры
  ...
  Future<void> _checkMinigameStatus() async {
    ...
    // Проверяем пасхалку версии и статус мини-игры
    ...
    final hideMinigame = prefs.getBool('hide_minigame') ?? false; // Загружаем настройку скрытия
    
    // Если пасхалка найдена или мини-игра уже разблокирована
    if ((eggVersionFound || minigameUnlocked) && !_minigameUnlocked) {
      setState(() {
        _minigameUnlocked = true;
        _hideMinigame = hideMinigame; // Учитываем настройку скрытия
        updateNavigation();
      });
  ...
  // Новый метод для обновления видимости мини-игры
  void updateMinigameVisibility(bool showMinigame) {
    setState(() {
      _hideMinigame = !showMinigame;
      updateNavigation();
    });
  }
  ...
  void updateNavigation() {
    ...
    setState(() {
      if (_minigameUnlocked && !_hideMinigame) {
        // Добавляем мини-игру в навигацию только если она не скрыта
        _screens = [
          const ScheduleScreen(),
          const CalendarScreen(),
      ...
      // Корректируем выбранный индекс, если он выходит за пределы доступных экранов
