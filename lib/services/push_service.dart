...
class PushService {
  ...
  static Future<bool> handleMessage(Map<String, String> data) async {
    ...
    // Хэш запоминается только после того, как синхронизация прошла.
    //
    // Раньше он сохранялся до неё, и сорвавшаяся синхронизация — телефон в
    // метро, нет сети — означала, что повторное сообщение о том же
    // изменении будет отброшено как дубликат. Изменение при этом пропадало
    // до ближайшего опроса по таймеру, то есть до того самого механизма,
    // от ненадёжности которого сторож и спасает.
    if (hash != null && hash.isNotEmpty) {
